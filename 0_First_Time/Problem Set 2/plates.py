            ...
            for i in range(2, len(s)):                          #checks each character after the first two in the input
                if s[i].isdigit() and (dig[0] != 0):            #checks if the character is a number and if the first number in the input isn't a zero
                    if s[i].isdigit() :                        #if the rest of the character after this one are ALL numbers, return True
                        print("test good", s[i:])
                        return True
                    elif s[i:].isalpha():                       #if there is a letter amongst the rest of the characters after the first number found, return False
                        print("test bad", s[i:])
                        return False
        else :         #if there is no number in the input, return True (while it meets the size requierments)
            return True
    else :
        return False
